...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    void Update() {
        ...
        if (_lerp) {
            ...
            scrollBar.value = Mathf.Lerp(scrollBar.value, _lerpTabTo, decelerate);
    ...
    private void SetPagePositions() {
        ...
        for (int i = 0; i < _pageCount; i++) {
            ...
            tabPosition = (float)i / (float)(_pageCount-1);
            child.anchoredPosition = childPosition;
            _pagePositions.Add(-childPosition);
            _tabPositions.Add(tabPosition);
        }
    }
    ...
    public void OnDrag(PointerEventData aEventData) {
        scrollBar.value -= (aEventData.delta.x/(float)Screen.width)/((float)_pageCount-1f);
        if (!_dragging) {
            // dragging started
            _dragging = true;
